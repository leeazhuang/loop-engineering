...
关键设计：**写代码的 AI 和验收的 AI 是两个不同角色**（绝不自己给自己打分），底下还压着**三道写死的安全脚本**（防烧钱、拦危险命令、卡"测试全绿才放行"）。所以它敢自动化，又不容易闯祸。
...
> ⚠️ 命令一定要填能真跑通的（比如后端 `pytest`、前端 `npm test`）。没填的那一侧循环会拒绝自动合并——"没验证 ≠ 通过"。

3. 顺手在 `CLAUDE.md` 的「项目规约」一节，写下本项目的命名/分层/测试约定（给 AI 读的自由文本）。

### 第 3 步 · 写需求文档（把活喂进来）

在项目根新建 `需求文档.md`，把客户需求写成一条条**功能 + 完成标准**。写得越具体，循环拆得越准。
