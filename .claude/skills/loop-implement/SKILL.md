...
1. **开隔离 worktree（原生 git worktree）**：先读 `.claude/loop.env` 取 `MAIN_BRANCH`，从最新主分支切出独立工作目录，互不踩脚。
   ...
   ```bash
   git fetch --quiet
   git worktree add ".worktrees/loop-<任务短名>" -b "loop/<任务短名>" "$MAIN_BRANCH"
   # 记录 worktree 绝对路径，供 gate-stop 全绿门进这棵树验本圈改动（而非主仓库旧树）。
   # 必须写主仓库的 .claude/memory（执行本步时 cwd 在主仓库根，相对路径即指向它）。
   ( cd ".worktrees/loop-<任务短名>" && pwd ) > .claude/memory/current-worktree
   ```
   - 分支名固定前缀 `loop/`，目录放在仓库内 `.worktrees/loop-<任务短名>`（已被 `.gitignore` 忽略）。**刻意放仓库内而非同级 `../`**：worktree 留在工作区根之下，Claude Code 不会因"访问工作区外目录"而弹确认框，全自动跑循环时无需人工点允许。
